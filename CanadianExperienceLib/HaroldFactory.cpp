...
std::shared_ptr<Actor> HaroldFactory::Create(std::wstring imagesDir)
{
    ...
    auto prop = make_shared<ImageDrawable>(L"Prop", imagesDir + L"/cig.png");
    prop->SetCenter(wxPoint(0,0));
    prop->SetPosition(wxPoint(20,-50));
    prop->SetRotation(-49.99);
    headb->AddChild(prop);
