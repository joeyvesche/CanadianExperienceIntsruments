...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::MachineSelect1, this, XRCID("Machine 1"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::MachineSelect2, this, XRCID("Machine 2"));
}
...
/**
 * Menu event handler for the first machine
 * @param event The menu event
 */
void ViewEdit::MachineSelect1(wxCommandEvent& event)
{

    GetPicture()->GetMachine1()->SetDialog(mFrame);
    UpdateObserver();

}

/**
 * Menu event handler for the second machine
 * @param event The menu event
 */
void ViewEdit::MachineSelect2(wxCommandEvent& event)
{

    GetPicture()->GetMachine2()->SetDialog(mFrame);
    UpdateObserver();
}
